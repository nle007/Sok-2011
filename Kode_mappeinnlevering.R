...
df <- subset(df_growth2019, select = c("avg_gdpgrowth", "avg_nsy", "avg_p", "avg_educ", "avg_gi", "avg_gx", "avg_nry", "gdppc0"))
...
labs <- c("Gjennomsnitlig årlig vekstrate i BNP pc 2000-2019 ",
          "Gjennomsnittlig sparing som andel av BNI (netto)",
          ...
          "BNP per inbygger i åar 2000")

#Lag tabellen
st(df, labels=labs)
st(df, vars =c("avg_gdpgrowth", "avg_nsy", "avg_p", "avg_educ", "avg_gi", "avg_gx", "avg_nry","gdppc0"))
st(df, labels=labs,
   summ = list(
...
# graf1: naturlige logaritmen av BNP per innbygger og sparing
df_growth2019 %>%
  ggplot(aes(x = avg_nsy, y = ln_gdppc)) +
  xlab("Sparing") + 
  ylab("Logaritme av BNP per inbygger 2019") + 
  geom_text(aes(label=iso3c),size=3)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  theme_classic()

# graf2: naturlige logaritmen av BNP per innbygger og befolkningsvekst
df_growth2019 %>%
...
# graf3: naturlige logaritmen av BNP per innbygger og humankapital
df_growth2019 %>%
  ggplot(aes(x = avg_educ, y = ln_gdppc)) +
  xlab("humankapital/snittutdanning") + 
  ylab("Logaritme av BNP per inbygger 2019") + 
  geom_text(aes(label=iso3c),size=3)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  theme_classic()

# graf4: årlig vekstrate i BNP per innbygger og sparing
df_growth2019 %>%
...
# graf5: årlig vekstrate i BNP per innbygger og utdanning
df_growth2019 %>%
...
gap_m %>% filter(year == 2007) %>% 
ggplot(aes(x = lifeExp, y = gdpPercap)) +
  xlab("Forventet levealder") + 
  ylab("BNP per innbygger år 2007") +
  labs(title = "BNP per innbygger og forventet levelader",
...
model1 <- lm(avg_gdpgrowth  ~ avg_gi + avg_nsy + avg_p + avg_educ + avg_gx + avg_nry + gdppc0 , data=  df_growth2019 ) #NB: Du må legge inn "Dine" variabler her
summary(model1)
...
#Q1educ <- quantile(df_growth2019$avg_educ, .25)
#Q3educ <- quantile(df_growth2019$avg_educ, .75)
#IQReduc <- IQR(df_growth2019$avg_educ)

no_outliers1 <- subset(df_growth2019, df_growth2019$avg_gi > (Q1gi - 1.5*IQRgi) & df_growth2019$avg_gi < (Q3gi + 1.5*IQRgi)) #&  df_growth2019$avg_educ > (Q1educ - 1.5*IQReduc) & df_growth2019$avg_educ < (Q3educ + 1.5*IQReduc))
dim(no_outliers1)

View (no_outliers1)
model2 <- lm(avg_gdpgrowth  ~ avg_gi + avg_nsy + avg_p + avg_educ + avg_gx + avg_nry + gdppc0 , data= no_outliers1) #NB: Du må legge inn "Dine" variabler her
summary(model2)
...
labs2 <- c("Vekst i BNP pc"," vekst i invisteringer", "Sparing som andel av BNI", "Befolkningsvekst", "Antall år utdanning", 
           "Veksrate i eksport", "Negativ veksrate i naturressurser", "BNP pc 2000")
